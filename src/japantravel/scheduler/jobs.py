...
def collect_job(context: PipelineContext | None = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if ctx.place_repo is not None and isinstance(items, list):
            upsert = ctx.place_repo.upsert_places(items, source="apify", source_id=ctx.settings.apify_actor_id)
            logger.info(
                "collect_job upserted places count=%s inserted=%s reused=%s errors=%s",
...
def collect_from_apify_dataset(
    ...
    try:
        ...
        upsert = ctx.place_repo.upsert_places(
            items,
            source="apify",
            source_id=ctx.settings.apify_actor_id,
        )
...
def _collect_image_urls(obj: Mapping[str, Any]) -> list[str]:
    raw_urls: list[str] = []
    for key in ("image_urls", "images", "photos", "image", "photo", "photosUrl", "photoUrl", "imageUrl"):
        value = obj.get(key)
        if value is None:
            continue
